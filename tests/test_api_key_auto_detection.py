...
def test_llm_client_auto_detects_openai_from_api_key(mock_openai):
    ...
    with patch.dict(os.environ, {}, clear=True), patch.dict(os.environ, {"API_KEY": "sk-test-key"}):
        client = LLMClient()
...
def test_llm_client_auto_detects_gemini_from_api_key(mock_openai):
    ...
    with (
        ...
        assert client.provider.llm == "gemini-3.1-flash-lite"
...
def test_llm_client_colab_api_key_detection():
    ...
    mock_userdata.get.side_effect = lambda key: (
        "sk-colab" if key == "API_KEY" else Exception("Not found")
    )
